// Package legacy is the 0.11 mush host: spawn `mush stdio` per run, keep the
// event stream in a ring buffer, forward client commands. Kept for one
// release behind MUXDECK_MUSH_LEGACY=1 while the ledger-backed viewer in
// package mushrun settles; the envelope shapes (mush docs/PROTOCOL.md) are
// the only contract.
package legacy
